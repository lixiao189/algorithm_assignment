...
int tabu_search(vector<Object> &objects, vector<Knapsack> &knapsacks,
  ...
  auto tabu_length = objects.size() / 2;
  auto iteration = 1000; // iteration times
  while (iteration--) {
    // Generate neighbors
    ...
    for (auto i = 0; i < objects.size(); i++) {
      if (solution[objects[i].index] == -1) {
        ...
        for (auto &knapsack : knapsacks) {
          if (knapsack.index != solution[objects[i].index] &&
              knapsack.capacity - knapsack.cost >= objects[i].w) {
            auto neighbor = solution;
            neighbor[objects[i].index] = knapsack.index;
            if (!in(tabu_list, neighbor)) {
    ...
    if (!in(tabu_list, solution)) {
      tabu_list.push_back(solution);
      if (tabu_list.size() > tabu_length) {
        solution = tabu_list.front();
        tabu_list.pop_front();
      }
    }
